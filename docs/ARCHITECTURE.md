...
public WS/REST -> normalize quote -> Motor -> risk/decision -> two-leg simulator
                                      |             |              |
                                      v             v              v
                                  GA strategy   simulated wallets  audit backend
                                      \             |              /
...
5. Audit records use SQLite locally and in the explicitly ephemeral public demo,
   or TLS-protected TimescaleDB in the durable production profile. The bounded
   worker drains on SIGTERM and exposes pending, dropped and failed writes.
   Preflight fails closed after any dropped or failed audit write.
...
Feeds and periodic analysis run as Tokio tasks behind `Arc<Motor>`. Execution reserves inventory by `(exchange, asset)`: incompatible routes are rejected while independent wallets can proceed in the same batch; the global lane is limited to multi-step demo/admin workflows. Broadcast is bounded; slow WebSocket clients may miss snapshots and recover on the next one. Market ingestion continues if a browser disconnects. The durable production profile fails readiness unless TimescaleDB reports persistent storage; the public evaluation profile may opt into SQLite only through `ALLOW_EPHEMERAL_PRODUCTION=true`, and exposes `storagePersistent=false`. Credentials are never returned in public state.
